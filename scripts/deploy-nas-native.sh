...
TARGET_DIR="/share/Container/vinyl-app-build"
...
tar -czf - --exclude="node_modules" --exclude=".git" --exclude="dist" --exclude="*.zip" --exclude="public/storage" --exclude=".gemini" --exclude="tmp_pb_sync" --exclude=".vercel" --exclude="backend/pb_data" --exclude="backend/pocketbase" . | ssh $NAS_USER@$NAS_IP "tar -xzf - -C $TARGET_DIR"

echo "⚙️  3. Costruisco e avvio l app direttamente sul NAS (Ti verra chiesta la password per il riavvio)..."
ssh -t $NAS_USER@$NAS_IP "bash -lc 'cd $TARGET_DIR && \
