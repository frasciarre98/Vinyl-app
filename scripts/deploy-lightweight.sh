...
echo "🚀 1. Avvio deploy lightweight V37.2 da: $(pwd)"
npx vite build --emptyOutDir

echo "🔍 Verifica build V37.2..."
if ! grep -r "V37.2" dist/assets/ > /dev/null 2>&1; then
    echo "❌ ERROR: La build prodotta non contiene la versione V37.2!"
    echo "   Attesa: V37.2-ULTIMATE nel codice sorgente."
    exit 1
fi
